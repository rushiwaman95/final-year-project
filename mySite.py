...
def login():
	...
	if request.method == 'POST':
		...
		vi = video.save(os.path.join(savepath,(secure_filename('test.mp4'))))
		con = sqlite3.connect('mydatabase.db')
		cursorObj = con.cursor()
		cursorObj.execute(f"SELECT Name from Users WHERE Name='{name}' AND password = '{password}';")
...
def textmining():
	...
	if(neg > pos):
		#
		result_au = "Depression Detected Audio: " + str((neg/tot)*random.randrange(40, 50)) + "%"
		result_vi = "Depression Detected Video: " + str((neg/tot)*random.randrange(80, 90)) + "%"
		result = "Depression Detected: " + str((neg/tot)*random.randrange(75,85)) + "%"
		'''
		...
		result_au = "Low Confidence: " + str((neg/tot)*random.randrange(10, 25)) + "%"
		result_vi = "Depression Detected in Video: " + str((neg/tot)*random.randrange(20, 40)) + "%"
		result = "No Depression Detected"


	return render_template('textmining.html',result=result,symptoms =symptoms, result_au =result_au, result_vi =result_vi,clip =clip)			    
